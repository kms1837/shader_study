...
void ofApp::setup() {
	...
	alphaTestShader.load("chapter6_passthrough.vert", "alpha_test.frag");
	cloudShader.load("chapter6_passthrough.vert", "cloud.frag");
...
void ofApp::update() {
	if (walkRight) {
		...
		// ofGetLastFrameTime: 이전 프레임 부터 현재 프레임까지 경과한 시간
...
void ofApp::draw() {
	...
	glm::mat4 defaultPosition= glm::translate(glm::vec3(0.f, 0.f, -0.5f));
	// z값을 -0.5를 하는 이유는 프러스텀이 0에서 -10에서 끝나게 설정했기 때문
	// 이외의 값을 넣으면 아예 안보이게됨
	...
	// 투영 행렬 1024 x 768기준 (1024/768 = 1.33)
...

	{
		...
		alphaTestShader.setUniformMatrix4f("model", defaultPosition);
		backgroundQuad.draw();
...

	{
		...
		cloudShader.setUniformMatrix4f("model", defaultPosition);
		sunQuad.draw();
